...
def filterbibtex(bib, name=None, year=None, tag=None, labels=None, category=None, fname=None):
    ...
    if not labels:
        ...
        for f in fname:
            ...
            if category:
                allrefs = allrefs.intersection(set(label[0] for label in filter(lambda e: e[1].type == category, bib[f].entries.items())))
            labels += list(allrefs)
...
files and filtering by name, year, tag, and label are supported."""
    ...
    def __init__(self, env, id):
        super(BibtexSupportPlugin, self).__init__(env, id)
    ...
    def list_publications(self, name=None, tag=None, year=None, labels=None, category=None, fname=None):
        bib = filterbibtex(self.bib_data, name=name, tag=tag, year=year, labels=labels, category=category, fname=fname)
